...
class Qwen2VLGRPOAnomalyTrainer(Trainer):
    """
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            if "Qwen2_VL" in model_id:
                model = Qwen2VLForConditionalGeneration.from_pretrained(model, **model_init_kwargs)
            elif "Qwen2_5_VL" in model_id:
        ...
        if processing_class is None:
            if "Qwen2_VL" in model_id or "Qwen2_5_VL" in model_id or "Aria" in model_id or True:
                processing_class = AutoProcessor.from_pretrained(model_id)
                pad_token_id = processing_class.tokenizer.pad_token_id
                processing_class.pad_token_id = pad_token_id
                processing_class.eos_token_id = processing_class.tokenizer.eos_token_id
                if "Qwen" in model_id or "Qwen2_5_VL" in model_id:
        ...
        self.anomaly_reward = script_args.anomaly_reward
        ...
        self.anomaly_num_generations = 1
        self.anomaly_generation_config = GenerationConfig(
            max_new_tokens=self.max_completion_length,
            do_sample=True,
            top_p=0.95,  
            temperature=1, # HACK
            num_return_sequences=self.anomaly_num_generations,
            pad_token_id=pad_token_id,
        )
    ...
    def extract_time(self, text):
        # pattern = r'<when>\[([0-9.]+)\s*,\s*([0-9.]+)\]</when>'
        pattern = r'<when>\[([+-]?\d*\.\d+|\d+)\s*,\s*([+-]?\d*\.\d+|\d+)\]</when>'
        match = re.search(pattern, text, re.DOTALL)

        if match:
            s = float(match.group(1).strip())
            e = float(match.group(2).strip())
            if s < e and s >= 0.0 and e <= 1.0:
                return [s, e]
        return None 
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # {'source': 'SH', 'video': '01_0015', 'anomaly_type': 'Abnormal', 'path': '/media/wbf/VA-Reasoning-RL/SH/Abnormal/01_0015.mp4', 'prompt': [{'content': [{'text': None, 'type': 'video'}, {'text': 'describe the video', 'type': 'text'}], 'role': 'user'}]}
    ...
    # 换成path即可
        ...
        try:
            # video_inputs: torch.Size([16, 3, 280, 392])
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            ...
            anomaly_rewards_per_func = torch.zeros(len(completions), 1, device=device)
            if self.anomaly_reward:

                all_anomaly_image_inputs = []
                all_anomaly_video_inputs = []
                all_prompt_text = []
                
                answer_time_range = []
                for j in range(len(completions)):

                    answer = self.extract_answer(completions[j][0]['content'])

                    ignored_intervals = self.extract_time(completions[j][0]['content'])

                    all_prompt_text.append(copy.deepcopy(prompts_text[0]))


                    if answer == "Abnormal" and ignored_intervals != None:
                    
                        
                        anomaly_image_inputs, anomaly_video_inputs, video_kwargs = process_vision_info(input_copy, return_video_kwargs=True, ignored_intervals=ignored_intervals)

                        answer_time_range.append(ignored_intervals)


                    else:
                        if random.random() < 0.5:
                            n_ignored_intervals = [0.0, 0.3]  
                        else:
                            n_ignored_intervals = [0.7, 1.0]  

                        anomaly_image_inputs, anomaly_video_inputs, video_kwargs = process_vision_info(input_copy, return_video_kwargs=True, ignored_intervals=n_ignored_intervals)
                        answer_time_range.append(n_ignored_intervals)

                    # all_anomaly_image_inputs.append(anomaly_image_inputs[0])
                    all_anomaly_video_inputs.append(anomaly_video_inputs[0])


                    # 2. input
                anomaly_prompt_inputs = self.processing_class(
                    text=all_prompt_text,
                    images=None,
                    videos=all_anomaly_video_inputs,
                    return_tensors="pt",
                    padding=True,
                    padding_side="left",
                    add_special_tokens=False,
                )
                anomaly_prompt_inputs = super()._prepare_inputs(anomaly_prompt_inputs)

                if self.max_prompt_length is not None:
                    anomaly_prompt_inputs["input_ids"] = anomaly_prompt_inputs["input_ids"][:, -self.max_prompt_length :]
                    anomaly_prompt_inputs["attention_mask"] = anomaly_prompt_inputs["attention_mask"][:, -self.max_prompt_length :]

                anomaly_prompt_ids, anomaly_prompt_mask = anomaly_prompt_inputs["input_ids"], anomaly_prompt_inputs["attention_mask"]

                if self.max_prompt_length is not None:
                    anomaly_prompt_ids = anomaly_prompt_ids[:, -self.max_prompt_length :]
                    anomaly_prompt_mask = anomaly_prompt_mask[:, -self.max_prompt_length :]
                    
                # 3. genrate
            
                anomaly_prompt_completion_ids = unwrapped_model.generate(**anomaly_prompt_inputs, generation_config=self.anomaly_generation_config)
                anomaly_prompt_length = anomaly_prompt_ids.size(1)
                anomaly_prompt_ids = anomaly_prompt_completion_ids[:, :anomaly_prompt_length]
                anomaly_completion_ids = anomaly_prompt_completion_ids[:, anomaly_prompt_length:]
                anomaly_prompt_mask = prompt_mask.repeat_interleave(self.anomaly_num_generations, dim=0)
                    
                # 4. decode
                anomaly_completions = self.processing_class.batch_decode(anomaly_completion_ids, skip_special_tokens=True)
                if is_conversational(inputs[0]):
                    anomaly_completions = [[{"role": "assistant", "content": anomaly_completion}] for anomaly_completion in anomaly_completions]
                    
                # Compute the rewards

                # 5. get new answer
                for k in range(len(anomaly_completions)):
                    new_answer = self.extract_answer(anomaly_completions[k][0]['content'])


                    # 6. reward
                    if answer == "Abnormal" and new_answer == "Normal":

                        rewards_per_func[k, 0] += 0.5

                    if answer == "Normal" and new_answer == "Abnormal":
                        if rewards_per_func[k, 0] > 0 or rewards_per_func[k, 1] > 0:
                            rewards_per_func[k, 0] += -0.2
        ...
        if self.len_control:
            ...
            mask = rewards_per_func[:, 0] >= 1.0
            lenth_list = completion_mask.sum(1)
            selected_indices = torch.nonzero(mask, as_tuple=True)[0].tolist()
                    
            if len(selected_indices) > 1:     
                for idx in selected_indices:
                    answer = self.extract_answer(completions[idx][0]['content'])
                    if answer == "Normal":
                        if 140 <= lenth_list[idx] <= 261:
                            rewards[idx] += 0.2
                    else:
                        if 233 <= lenth_list[idx] <= 456:
                            rewards[idx] += 0.2
