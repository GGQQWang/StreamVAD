...
gt_path = "./Vad-Reasoning-SFT-test.jsonl"

output_file = f"./result.jsonl"
...
i = 0
with open(gt_path, 'r', encoding='utf-8') as infile, open(output_file, 'w', encoding='utf-8') as outfile:
    for line in infile:
        ...
        output_data = {
            ...
            "output": result[0].outputs[0].text  # 假设模型输出的文本在索引 0
        }

        json.dump(output_data, outfile, ensure_ascii=False)
        outfile.write("\n")
        i += 1
        print(i, data.get("video"))     
